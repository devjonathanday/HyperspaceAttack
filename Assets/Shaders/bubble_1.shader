...
Shader "Shader Forge/bubble_1" {
    Properties {
        ...
        [HDR]_node_2181 ("node_2181", Color) = (0.5,0.5,0.5,1)
    }
    SubShader {
        Tags {
...
////// Emissive:
                ...
                float3 emissive = lerp(saturate(sceneColor.rgb),_node_2181.rgb,saturate((_texture_var.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_fresnelstrength)*_transparency)));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float2 node_8731 = ((i.uv0+(node_3162.g*_speed)*float2(0,1))+(node_3162.g*_speed)*float2(1,0));
                float4 _texture_var = tex2D(_texture,TRANSFORM_TEX(node_8731, _texture));
                o.Emission = lerp(saturate(sceneColor.rgb),_node_2181.rgb,saturate((_texture_var.rgb*pow(1.0-max(0,dot(normalDirection, viewDirection)),_fresnelstrength)*_transparency)));
